      ...
      {gridData.map((row, rowIndex) => (
        <div key={rowIndex} className="flex flex-row">
          {row.map((cell, cellIndex) => {
            let paletteIndex: number | null;
            let fillType: string = "full";

            if (typeof cell === "string") {
              const parts = cell.split(":");
              paletteIndex = Number(parts[0]);
              fillType = parts[1] || "full";
            } else {
              paletteIndex = cell;
            }
            ...
            return (
              <div
                ...
                style={{
                  width: cellSize,
                  height: cellSize,
                  backgroundColor: fillType === "full" ? color : "white",
                  borderTop:
                    rowIndex === centerRow
                      ? "1px solid red"
                      : isTenth(rowIndex)
...
const PrintBody: FC<{
  ...
  showLogo: boolean;
}> = ({ name, gridData, palette, orientation, showColorGuide, showCopyright, showLogo }) => (
    ...
    {(showCopyright || showLogo) && (
      <footer>
        <div className="w-full flex justify-between items-end">
          ...
          {showLogo && <img src={logo} alt="logo" style={{ height: "2.5em" }} />}
        </div>
      </footer>
    )}
  </div>
              ...
              <div className="w-64 m-4 flex flex-col space-y-4">
                ...
                <label className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => setShowColorGuide(e.target.checked)}
                  />
                  <span>Show color guide</span>
                </label>
                <label className="flex items-center space-x-2">
                  <input
                    type="checkbox"
